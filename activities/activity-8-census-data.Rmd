...
```{r, eval = F}
census_api_key("b3c5304d552e16e473b315db0ad7302455075d08", install = TRUE)
```
...
or_county %>%
  st_as_sf() %>%
  mutate('percent_whiteE' = white_popE/total_popE) %>%
  ggplot() +
  geom_sf(aes(fill = percent_whiteE)) +
  scale_fill_viridis_c() +
  labs(fill = "% of Population White") +
  theme_void()
```
...
ca <- get_acs(geography = "tract", 
              year = 2022,
              variables = c(housing_value = "B25077_001"), 
              county = c("Los Angeles", "Ventura", "San Bernardino"),
              state = "CA",
              survey = "acs5",
              output = "wide")
...
housing_county <- left_join(ca, ca_sf, by = c("GEOID" = "GEOID"))

housing_county %>%
  st_as_sf() %>%
  ggplot() +
  geom_sf(aes(fill = housing_valueE)) +
  scale_fill_viridis_c() +
  labs(fill = "Cost") +
  theme_void()
